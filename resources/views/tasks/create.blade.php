<x-layouts.app title="New Task">
    ...
    <div class="max-w-2xl mx-auto">
        <div class="flex items-center gap-3 mb-6">
            ...
            <h1 class="text-2xl font-semibold text-gray-800">New Task</h1>
        </div>
                ...
                <select name="goal_id"
                    ...
                    @foreach ($goals as $goal)
                        <option value="{{ $goal->goal_id }}"
                            {{ (old('goal_id', $selectedGoalId) == $goal->goal_id) ? 'selected' : '' }}>
                            {{ $goal->title }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <select name="period"
                            class="w-full px-4 py-2.5 rounded-lg border border-gray-200
                                   focus:outline-none focus:ring-2 focus:ring-indigo-400 text-sm">
                        <option value="day"   {{ old('period') === 'day'   ? 'selected' : '' }}>Daily</option>
                        <option value="week"  {{ old('period') === 'week'  ? 'selected' : '' }}>Weekly</option>
