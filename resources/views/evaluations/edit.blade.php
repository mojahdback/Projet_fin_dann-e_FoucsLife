...
<x-layouts.app title="Edit Evaluation">
    ...
    <div class="max-w-2xl mx-auto space-y-6">
        ...
        <div class="bg-white border border-gray-200 rounded-lg shadow-sm">
            <div class="p-6 border-b border-gray-200">
                <h1 class="text-2xl font-bold text-gray-900">Edit Self Evaluation</h1>
                <p class="text-gray-600 mt-1">Update your evaluation score and reflection</p>
            </div>
            ...
            <form action="{{ route('evaluations.update', $evaluation->evaluation_id) }}" method="POST"
                ...
                <div>
                    ...
                    <select id="period_type" name="period_type" required
                        ...
                        <option value="day" {{ $evaluation->period_type === 'day' ? 'selected' : '' }}>Daily</option>
                        <option value="week" {{ $evaluation->period_type === 'week' ? 'selected' : '' }}>Weekly</option>
                        <option value="month" {{ $evaluation->period_type === 'month' ? 'selected' : '' }}>Monthly
                        </option>
                        <option value="year" {{ $evaluation->period_type === 'year' ? 'selected' : '' }}>Yearly</option>
                    </select>
                    @error('period_type')
                ...
                <div>
                    ...
                    <input type="date" id="period_date" name="period_date" required
                        value="{{ old('period_date', \Carbon\Carbon::parse($evaluation->period_date)->format('Y-m-d')) }}"
                        class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                    @error('period_date')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                        ...
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-2">
                                <span id="scoreDisplay"
                                    class="text-3xl font-bold text-indigo-600">{{ old('score', $evaluation->score) }}</span>
                                <span class="text-gray-500">/10</span>
                            </div>
                            <div id="scoreLabel" class="text-sm font-medium px-3 py-1 rounded-full">
                ...
                <div>
                    ...
                    <textarea id="comment" name="comment" rows="4"
                        ...
                        class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">{{ old('comment', $evaluation->comment) }}</textarea>
